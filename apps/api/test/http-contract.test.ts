...
function authenticatedApp():{instance:ReturnType<typeof buildApp>;token:string;query:ReturnType<typeof vi.fn>}{
  ...
  const query=vi.fn((sql:string,values?:unknown[])=>{
    ...
    if(sql.includes('INSERT INTO cadastros.cost_centers')){expect(values).not.toContain('');return {rows:[{id:'33333333-3333-4333-8333-333333333333',parent_id:null,code:'TEST',name:'Teste',description:'Descricao',is_active:true,created_at:new Date(),updated_at:new Date()}],rowCount:1};}
...
describe('HTTP contracts',()=>{
  ...
  it('protects financial routes without a bearer token',async()=>{const payment=await app().inject({method:'POST',url:'/api/v1/payments',payload:{}});expect(payment.statusCode).toBe(401);expect(payment.payload).toContain('UNAUTHORIZED');
    const eligible=await app().inject({method:'GET',url:'/api/v1/payable-installments/eligible-for-payment'});expect(eligible.statusCode).toBe(401);expect(eligible.payload).toContain('UNAUTHORIZED');});
  ...
  it('accepts active query filters used by frontend lookups',async()=>{const {instance,token}=authenticatedApp();for(const path of ['companies','company-parameters','financial-categories','document-types']){const response=await instance.inject({method:'GET',url:`/api/v1/${path}?pageSize=100&active=true`,headers:{authorization:`Bearer ${token}`}});expect(response.statusCode).toBe(200);expect(response.json()).toMatchObject({data:[],page:1,pageSize:100,total:0});}});
  it('normalizes empty optional parent selectors on cost centers',async()=>{const {instance,token}=authenticatedApp();const response=await instance.inject({method:'POST',url:'/api/v1/cost-centers',headers:{authorization:`Bearer ${token}`},payload:{code:'TEST',name:'Teste',description:'Descricao',parentId:'',isActive:true}});expect(response.statusCode).toBe(201);expect(response.json()).toMatchObject({parentId:null,description:'Descricao'});});
