...
describe('HTTP contracts',()=>{
  ...
  it('accepts active query filters used by frontend lookups',async()=>{const {instance,token}=authenticatedApp();const response=await instance.inject({method:'GET',url:'/api/v1/financial-categories?pageSize=100&active=true',headers:{authorization:`Bearer ${token}`}});expect(response.statusCode).toBe(200);expect(response.json()).toMatchObject({data:[],page:1,pageSize:100,total:0});});
