  ...
  const save = useMutation({
    mutationFn: async (values: FormState) => {
      const payload = {
        ...
        companies: values.companies.map(({ companyId, isDefault, accessScope }) => ({ companyId, isDefault, accessScope })),
      };
  ...
  return (
    <div className="grid gap-6">
      <header className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <Button onClick={() => setForm(emptyForm)}>+ Novo usuário</Button>
      </header>
              ...
              <tbody>
                {users.data?.data.map((user) => (
                  <tr key={user.id} className="border-b border-slate-100 last:border-0">
                    ...
                    <td className="whitespace-nowrap px-3 py-3 text-right"><button className="font-semibold text-blue-700 hover:underline" onClick={() => openEdit(user)}>Editar</button>{user.isActive ? <button className="ml-4 font-semibold text-red-700 hover:underline" onClick={() => { if (window.confirm('Inativar usuário?')) deactivate.mutate(user); }}>Inativar</button> : <button className="ml-4 font-semibold text-emerald-700 hover:underline" onClick={() => reactivate.mutate(user)}>Reativar</button>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="fixed inset-0 z-30 grid place-items-center bg-slate-950/50 p-4" role="dialog" aria-modal="true" aria-label={form.id ? 'Editar usuário' : 'Novo usuário'}>
          <form className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-3xl bg-white p-6 shadow-2xl" onSubmit={(event) => void submit(event)}>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="grid gap-1.5 text-sm font-semibold text-slate-700">Senha {form.id ? '(preencha apenas para trocar)' : ''}<input aria-label="Senha" required={!form.id} minLength={8} type="password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} className="min-h-11 rounded-xl border border-slate-300 px-3" /></label>
              <div className="grid gap-2 rounded-2xl border border-slate-200 p-4">
                <label className="flex items-center gap-3 text-sm font-bold text-slate-700"><input type="checkbox" checked={form.isActive} onChange={(event) => setForm({ ...form, isActive: event.target.checked })} className="size-4 accent-blue-700" />Usuário ativo</label>
                <label className="flex items-center gap-3 text-sm font-bold text-slate-700"><input type="checkbox" checked={form.isMaster} onChange={(event) => setForm({ ...form, isMaster: event.target.checked })} className="size-4 accent-blue-700" />Operador Master</label>
              </div>
            </div>
            <section className="mt-6 grid gap-4 lg:grid-cols-2">
              <div className="rounded-2xl border border-slate-200 p-4">
                ...
                <div className="mt-3 grid gap-2">
                  {roles.data?.map((role) => <label key={role.id} className="flex items-center gap-3 rounded-xl bg-slate-50 px-3 py-2 text-sm font-semibold"><input type="checkbox" checked={form.roleIds.includes(role.id)} onChange={(event) => toggleRole(role.id, event.target.checked)} className="size-4 accent-blue-700" />{role.name}</label>)}
                </div>
              </div>
              <div className="rounded-2xl border border-slate-200 p-4">
                ...
                <div className="mt-3 grid gap-2">
                  {companies.data?.map((company) => {
                    ...
                    return <div key={company.id} className="rounded-xl bg-slate-50 px-3 py-2"><label className="flex items-center gap-3 text-sm font-semibold"><input type="checkbox" checked={Boolean(selected)} onChange={(event) => toggleCompany(company, event.target.checked)} className="size-4 accent-blue-700" />{companyLabel(company)}</label>{selected ? <div className="mt-2 grid gap-2 pl-7 sm:grid-cols-2"><label className="flex items-center gap-2 text-xs font-semibold text-slate-600"><input type="radio" name="default-company" checked={selected.isDefault} onChange={() => updateCompany(company.id, { isDefault: true })} />Padrão</label><select aria-label={`Escopo ${companyLabel(company)}`} value={selected.accessScope} onChange={(event) => updateCompany(company.id, { accessScope: event.target.value as UserCompany['accessScope'] })} className="min-h-9 rounded-lg border border-slate-300 bg-white px-2 text-xs"><option value="OPERATIONAL">Operacional</option><option value="VIEW_ONLY">Somente leitura</option></select></div> : null}</div>;
                  })}
                </div>
              </div>
            </section>
