...
SELECT
    count(*) FILTER (WHERE document_number LIKE 'LOCAL-%') AS seeded_titles,
    count(*) FILTER (WHERE legal_name IN ('Oficina Pinhais LTDA', 'Auto Pecas Serra Azul SA')) AS seeded_suppliers
FROM financeiro.payable_titles t
FULL JOIN cadastros.suppliers s ON s.id = t.supplier_id;
